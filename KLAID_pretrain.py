...
def train(model, data_loader, optimizer, tokenizer, epoch, warmup_steps, device, scheduler, config, fp16_scaler, writer):
    ...
    for i, property_data_dict in enumerate(tqdm_data_loader):
        text = property_data_dict['fact']
        prompt_label = property_data_dict['laws_service']

        optimizer.zero_grad()
        text_input = tokenizer(text, padding='max_length', truncation=True, max_length=20, return_tensors="pt").to(device)
        label_intput = tokenizer(prompt_label, padding='longest', truncation=True, max_length=20, return_tensors="pt").to(device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', default='E:\\STUDY\KAIST_BISPL\\ALBEF_2textencoder\\KLAID_1\\')
